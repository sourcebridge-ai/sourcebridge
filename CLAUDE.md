...
**2026-06-12 GrailsRole persistence + QA language parity for ruby/php/csharp/cpp (CA-549)** — 4 commits, `4249c62d/b45cb375/ae6d6480/fa5f1abc`.
Follow-up to the CA-548 Groovy/Grails port. Two items deferred from that campaign: (1) `GrailsRole` was set at parse time on `indexer.FileResult` but dropped at persistence; entry-point classification re-derived it from the file path on every MCP call. Migration 061 adds `grails_role` to `ca_file`; `buildIndexBatch` writes it on both `StoreIndexResult` and `ReplaceIndexResult` paths; `surrealFile`/`toFile()` reads it back. All three in-memory `graph.File` constructors (Store, Replace, Merge) set the field. `mcp_entry_points.go` reads the stored value first and falls back to `grailsRoleFromPath(f.Path)` for pre-migration rows — behavior is byte-identical to before for un-reindexed repos. (2) `languageFromString`, `extFromPath`, and `SupportedExts` in the QA pipeline now cover ruby, php, csharp, and cpp alongside groovy; `test_finder.go` parity is deferred.

Load-bearing constraints for future-Claude:

- **Migration 061 form: single bare `DEFINE FIELD IF NOT EXISTS grails_role ON ca_file TYPE string DEFAULT '';` — no embedded `UPSERT schema_version:v61` marker.** The migration runner (`internal/db/surreal.go:333`) self-records the applied version via `CREATE schema_version SET version = $v, applied_at = time::now()` after the file's SQL succeeds; it keys gating off the filename numeric prefix, not off any marker embedded in the SQL. An embedded marker would create a duplicate version-61 record. Migration 060 (`060_llm_profile_created_via.surql` — a single bare `DEFINE FIELD` line) is the precedent to mirror. Do not add embedded markers to future `ca_file` field migrations.
- **`ca_file` is SCHEMAFULL — every field `buildIndexBatch` writes needs a `DEFINE FIELD` in a migration or SurrealDB silently drops it.** `ai_score` and `ai_signals` are written by `buildIndexBatch` today but have no `DEFINE FIELD` and are currently dropped (pre-existing bug, follow-up pending — not in CA-549 scope). `grails_role` is safe because migration 061 defines it. A code comment in `buildIndexBatch` (`internal/db/index_result.go:65-69`) documents this contract; preserve it.
- **`buildIndexBatch` is the single `ca_file` write site, shared by `StoreIndexResult` and `ReplaceIndexResult` (CA-304 structural parity).** A single edit to that function covers both write paths. `SurrealStore.MergeIndexResult` is a fail-closed stub and writes nothing to the DB — no DB-side change is needed there.
- **All three in-memory `graph.File` constructors must set `GrailsRole: fr.GrailsRole`.** The three sites are `StoreIndexResult` (`internal/graph/store.go:329`), `ReplaceIndexResult` (`:488`), and `MergeIndexResult` (`:740`). `MergeIndexResult` is a live insert path used by change-watch/delta flows; it was missed in the first plan draft. Any future `graph.File` field added at parse time must be wired at all three sites. `TestMergeIndexResult_GrailsRolePropagates` is the regression gate for the third constructor.
- **`surrealFile`/`toFile()` (`internal/db/helpers.go:226/238`) is the single `graph.File` read mapper.** Both `GetFiles` and `GetFilesPaginated` use `SELECT *` piped through this mapper; one edit covers both readers. Other `ca_file` readers (`SearchContent`, analytics counts, import/package-dependency joins) select for path/ID metadata and do not materialize `graph.File` — they do not need `grails_role`.
- **Stored-first/path-fallback is the `entrypoints.File` read contract in `mcp_entry_points.go`.** When `f.GrailsRole != ""`, use the stored value; when empty, call `grailsRoleFromPath(f.Path)`. Any future consumer that builds an `entrypoints.File` from a `graph.File` must implement the same fallback — a consumer that reads `f.GrailsRole` unconditionally silently regresses Grails classification for pre-migration and un-reindexed rows. The fallback is not an MCP-local quirk; it is the read contract until every install has reindexed. `grailsRoleFromPath` is retained as the fallback; do not remove it.
- **`languageFromString` matches language names only, never extension aliases (CA-548 Groovy precedent).** The new cases are `case "ruby":`, `case "php":`, `case "csharp":`, `case "cpp":` — exactly the names `DetectLanguage` emits. Do not add `"rb"`, `"cs"`, `"cc"`, `"cxx"`, `"c++"` etc. as aliases; that broadens the contract beyond what `DetectLanguage` produces and was explicitly rejected in plan review. The pre-existing `"py"`/`"ts"`/`"tsx"`/`"rs"` aliases on older cases are legacy inconsistencies, not a pattern to extend. Extension-to-language mapping belongs in `extFromPath` and `SupportedExts`, not `languageFromString`.
- **`TestGrailsRolePersistsAfterStoreAndReplace` (`internal/db/grails_role_persistence_integration_test.go`) is the SCHEMAFULL round-trip regression gate.** Two checkpoints: `grails_role == "grails_controller"` after `StoreIndexResult` AND after `ReplaceIndexResult`; non-Grails file `== ""` at both. This is the only proof that migration 061 actually landed the field and `buildIndexBatch`/`surrealFile`/`toFile()` carry it end-to-end. Do not delete or weaken it.
- **`test_finder.go` language parity for php/csharp/cpp is deferred to CA-TBD-language-coverage-parity.** That work requires per-ecosystem test-file naming convention design (PHPUnit, NUnit/xUnit, GoogleTest), adjacent-test directory layouts, and `endOfBlock` grammar handling for three new languages — not a mechanical backfill. The CA-549 Item-2 contract is `languageFromString` + `extFromPath` + `SupportedExts` only.

Plan: `thoughts/shared/plans/finished/2026-06-12-deliver-grails-role-persistence-language-parity.md`
Plane ticket: CA-549
...
- **`Config.MCP.PublicProbeEnabled` defaults `true`; disabled-state returns `404` (NOT `401`).** `web/src/lib/use-server-capabilities.ts:98-114` treats any status other than 404 as "MCP enabled." Returning 401 on disabled would make the frontend show MCP as enabled while actual MCP requests fail auth — confusing UX. Don't change the disabled status code to anything other than 404. Important: MCP itself is opt-in via `Config.MCP.Enabled` (default `false`). When `MCP.Enabled` is false, ALL MCP routes — including the HEAD probe — return 404 regardless of `PublicProbeEnabled`. The `PublicProbeEnabled` flag only differentiates HEAD probe behavior within an MCP-enabled install. A 404 on the HEAD probe does NOT imply `PublicProbeEnabled=false`; it may simply mean the entire MCP subsystem is disabled.
